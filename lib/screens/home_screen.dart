...
// Show the welcome message immediately so the initial UI does not depend
// on native TTS or SQLite initialization completing first.
_appendMessage(
...
```
// Native services initialize in the background.
await _tts.initialize();

// Restore persisted history when available.
try {
...
if (!ready) {
  if (mounted) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'Could not access the microphone / speech engine.',
        ),
      ),
    );
  }
...
ScaffoldMessenger.of(context).showSnackBar(
  SnackBar(
    ...
    content: Text(
      newVal ? 'Voice output ON' : 'Voice output OFF',
    ),
  ),
);
```
